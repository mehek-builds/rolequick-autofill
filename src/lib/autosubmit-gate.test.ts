...
import { workAuthSkipReason } from './adapters/generic';
...
describe('skippedReasonsNeedReview', () => {
  ...
  it('holds on a work-authorization question left for the student (live QA 2026-07-16 fix)', () => {
    // Work-auth questions are never auto-answered anymore; the adapters emit exactly this reason,
    // and auto-submit must not fire while the question sits unanswered.
    expect(
      skippedReasonsNeedReview([
        'work-authorization question left for you: "Are you legally authorized to work in the locat"',
      ]),
    ).toBe(true);
  });

  it('pins the shared reason builder to the gate: workAuthSkipReason output must always hold', () => {
    // The hold rests on this string contract. If someone rewords workAuthSkipReason so it stops
    // matching REVIEW_FLAG, auto-submit could fire with a legal question blank; this test is the
    // tripwire.
    expect(skippedReasonsNeedReview([workAuthSkipReason('are you authorized to work in the us?')])).toBe(true);
  });
